...
### 3. Сборка отчёта + CRM (код, не руками)

⛔ **Не вызывать** `crm.lead.update` / `im.message.add` вручную.

```bash
# после judgements.jsonl:
LEAD_ANALYSIS_MODE=dry-run|live python3 scripts/deliver_report.py
# RUN_DIR по умолчанию .run/latest
```

Скрипт сам:
- мержит `triage.crm_auto_by_id` (F04 из SOURCE_ID) + `crm_updates` из auto/judgements;
- в **live** пишет только **пустые** UF, шлёт отчёт в chat31598, кнопки бота, CSV;
- в **dry-run** пишет `report.txt` + `crm_stats.json`, Bitrix не меняет (кроме GET при наличии вебхука).

Шаблон языка — `references/report_format.md` (один раз перед проверкой текста).

### 4. Live / dry-run

| Режим | Поведение |
|---|---|
| `dry-run` | `report.txt` + лог; CRM/chat/CSV не писать |
| `live` | через `deliver_report.py`: CRM + chat31598 + кнопки + CSV |
...
3. Почему это важно? (перевести structural: `r02_human`, `r06_human`, `r08_human`, f03)
...
| ⚠️ action | R02/R06/F03, неверная стадия, дата контакта, процесс (не «перезвонить» на R08) |
...
### CRM (через deliver_report, не руками)

В `judgements.jsonl` → `crm_updates` класть только то, что **очевидно** из разговора и ещё пусто:
ТегиКЦ, Вариация, Препарат, Статус детальный, Задача, **Кто рекомендовал** (+ комментарий).

**Кто рекомендовал** (`UF_CRM_1711919555`):
- из **SOURCE_ID** (18/13/21/…) triage уже кладёт в `crm_auto_by_id` — в judgement можно не дублировать;
- из **разговора** (спросили — не назвал → `474`; повторный → `470`/`472`) — только в `crm_updates`;
- сомнение → не писать.
...
| R08 | сервисный номер (ПроДокторов и др.) | не предлагать перезвон на номер лида; не смешивать историю контакта; R02 не применяется |
| F03 | «Отказ пациента» только при явном отказе |
| F01 | препарат упомянут → не «Не применимо» |
| F04 | Кто рекомендовал: только если пусто; из разговора или однозначного SOURCE_ID; спросили — не назвал → 474 |
